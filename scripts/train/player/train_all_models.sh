...
CHECKPOINT_ROOT="${PROJECT_ROOT}/checkpoints/player"
HYDRA_OUTPUT_ROOT="${PROJECT_ROOT}/outputs/train_player"
...
BATCH_LOG_FILE="${LOG_DIR}/batch_training_player_${TIMESTAMP}.log"
...
log_environment() {
    ...
    local config_file="${PROJECT_ROOT}/configs/train/player/${config_name}.yaml"
...
train_and_archive_model() {
    ...
    if ! python -m src.player.api.train \
        --config-path="${PROJECT_ROOT}/configs/train/player" \
...
check_prerequisites() {
    ...
    if ! python -c "import src.player.api.train" 2>/dev/null; then
        print_error "Training module src.player.api.train not found"
        exit 1
    fi
    
    # Check if config directory exists
    if [ ! -d "${PROJECT_ROOT}/configs/train/player" ]; then
        print_error "Config directory not found: ${PROJECT_ROOT}/configs/train/player"
        exit 1
    fi
...
print_info "Starting batch training for all player models..."
