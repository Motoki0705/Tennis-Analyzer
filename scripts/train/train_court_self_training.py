...
from src.court.self_training.court_refiner import CourtRefiner
from src.court.self_training.self_training_cycle import CourtSelfTrainingCycle
...
def self_training(cfg: DictConfig) -> None:
    ...
    # 自己学習サイクルのインスタンス化
    cycle = CourtSelfTrainingCycle(
        max_cycles=cfg.self_training.max_cycles,
        confidence_threshold=cfg.self_training.confidence_threshold,
        similarity_threshold=cfg.self_training.similarity_threshold,
        max_keypoint_shift=cfg.self_training.max_keypoint_shift,
        min_high_quality_frames=cfg.self_training.min_high_quality_frames,
        output_dir=os.path.join("outputs", "court", "self_training"),
    )
    ...
    for i in range(cfg.self_training.max_cycles):
        ...
        with torch.no_grad():
            for batch in unlabeled_dataloader:
                ...
                # ヒートマップからキーポイントを抽出
                keypoints, scores = lit_module.extract_coordinates_from_heatmap(outputs)
                ...
                for i in range(len(batch_image_ids)):
                    ...
                    confidences.append(float(np.mean(scores[i])))
        
        # コート洗練器で擬似ラベルを洗練
        refiner = CourtRefiner(
            similarity_threshold=cfg.self_training.similarity_threshold,
            max_keypoint_shift=cfg.self_training.max_keypoint_shift,
            min_high_quality_frames=cfg.self_training.min_high_quality_frames,
        )
        
        refined_predictions = refiner.refine_predictions(predictions)
...
@hydra.main(config_path="../../configs/train/court", config_name="self_training_config", version_base="1.3")
