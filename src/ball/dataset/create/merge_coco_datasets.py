...
# --- 設定 ---
ORIGINAL_COCO_PATH = (
    "data/annotation_jsons/coco_annotations.json"  # ボール情報など元のデータ
)
...
class CocoMerger:
    ...
    def merge_info_licenses_images(self):
        ...
        self.merged_data["images"] = self.original_data.get(
            "images", []
        )  # 画像情報は元データにあるはず
        print(
            f"Using info, licenses, and {len(self.merged_data['images'])} images from original data."
        )
    ...
    def merge_categories(self):
        ...
        if person_cat_id is not None and person_cat_id != PERSON_DETECTION_CATEGORY_ID:
            print(
                f"Warning: Person detection file uses category ID {person_cat_id}, expected {PERSON_DETECTION_CATEGORY_ID}. Using ID {person_cat_id} from file."
            )
            ...
            REQUIRED_CATEGORIES[person_cat_id] = {
                "name": "non_player_person",
        ...
        for req_id, req_info in REQUIRED_CATEGORIES.items():
            if req_id not in merged_categories:
                print(
                    f"Info: Required category ID {req_id} ('{req_info['name']}') not found. Adding."
                )
        ...
        self.merged_data["categories"] = sorted(
            list(merged_categories.values()), key=lambda x: x["id"]
        )
    ...
    def merge_annotations(self):
        ...
        print(
            f"Re-assigning IDs for {len(person_annotations)} person annotations starting from {next_annotation_id}..."
        )
        reassigned_count = 0
        for ann in person_annotations:
            ...
            if new_ann.get("category_id") != PERSON_DETECTION_CATEGORY_ID:
                print(
                    f"Warning: Annotation ID {original_ann_id} has unexpected category ID {new_ann.get('category_id')}, expected {PERSON_DETECTION_CATEGORY_ID}. Keeping original category ID."
                )
                # 強制的に上書きする場合:
