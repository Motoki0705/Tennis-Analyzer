...
class FinalDatasetGenerator:
    ...
    def load_data(self) -> bool:
        ...
        if not self.input_merged_coco_path.is_file():
            print(f"Error: Input merged COCO file not found at {self.input_merged_coco_path}")
            return False
        ...
        existing_categories = {cat['id']: cat for cat in self.master_coco_data['categories']}
        required_categories = {
        ...
        if missing_category_names:
             print(f"Error: Input COCO JSON must contain the following categories: {', '.join(missing_category_names)}")
             return False
        ...
        print(f"Built annotation_id_to_image_id map for {len(annotation_id_to_image_id)} annotations.")


        # --- 分類結果ファイル群 ロードと集計 ---
        print(f"Loading classification results from: {self.spec_dir}")
        if not self.spec_dir.is_dir():
             print(f"Warning: Specification directory not found at {self.spec_dir}. No player/ignore/verification classifications will be applied.")
            ...
            try:
                ...
                for ann_id in player_ids_in_file:
                     ...
                     if img_id is not None:
                          ...
                          print(f"Warning: Player annotation ID {ann_id} found in {spec_file} but not in master COCO annotations. Cannot determine image_id for verification.")

            except Exception as e:
                print(f"Warning: Could not load or parse specification file {spec_file}: {e}. Skipping.")
        ...
        print(f"Loaded total {len(self.all_player_ids)} player IDs and {len(self.all_ignored_ids)} ignored IDs.")
        print(f"Determined {len(self.verified_image_ids)} image IDs as human verified (based on Player annotations).")
        return True
    ...
    def generate_final_annotations(self) -> List[Dict]:
        ...
        for ann in original_annotations:
            ...
            if ann_id is None or cat_id is None or img_id is None:
                 print(f"Warning: Skipping annotation with missing id, category_id, or image_id: {ann}. ")
            ...
            # その他のカテゴリは破棄
                 ...
                 print(f"Warning: Skipping annotation with unexpected category ID {cat_id}: {ann_id}. ")
                 discarded_count += 1
        ...
        print(f"  Category changed to Player ({CAT_ID_PLAYER}): {category_changed_to_player_count}")
        print(f"  Kept as NonPlayer ({CAT_ID_NON_PLAYER}): {kept_as_nonplayer_count}")
        print(f"  Discarded (Ignored or other): {discarded_count}")
        print(f"  Total annotations marked as human verified: {annotations_marked_verified_count} (belonging to {len(self.verified_image_ids)} images)") # ログ表示を修正
    ...
    def save_final_coco(self, final_annotations: List[Dict]):
        ...
        if not self.master_coco_data: print("Error: Master COCO data not loaded. Cannot save."); return
        ...
        try:
            ...
            with open(self.output_final_coco_path, 'w', encoding='utf-8') as f: json.dump(final_coco_data, f, indent=4)
