...
class CourtPredictor:
    ...
    def overlay_heatmap(self, frame: np.ndarray, hm: np.ndarray) -> np.ndarray:
        ...
        if hm_vis.max() > 0:
            ...
            color = cv2.resize(
                color, (frame.shape[1], frame.shape[0]), interpolation=cv2.INTER_LINEAR
            )
    ...
    def run(
        ...
        if self.visualize_mode != "heatmap_channels":
            ...
            writers = [
                cv2.VideoWriter(
                    str(parent / f"{stem}_kp{c}.mp4"), fourcc, fps, (width, height)
                )
                for c in range(self.num_keypoints)
            ]
        ...
        with tqdm(total=total, desc="Court 推論") as pbar:
            while True:
                ...
                if len(batch_frames) == batch_size:
                    ...
                    for frm, kps, hm in zip(batch_frames, kps_batch, hm_batch, strict=False):
