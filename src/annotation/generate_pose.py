...
def flatten_keypoints(
    kps_xy: List[Tuple[int,int]],
    scores: List[float],
    vis_thresh: float
) -> Tuple[List[float], int]:
    flat = []
    cnt  = 0
    for (x, y), s in zip(kps_xy, scores):
...
def main(cfg: DictConfig):
    ...
    det_model     = load_model_weights(
        det_model,
        to_absolute_path(cfg.det_checkpoint)
    )
    ...
    for frames, ids in loader:
        ...
        for img_id, dets in zip(ids, results):
            for d in dets:
                bbox   = d["bbox"]
                area   = bbox[2] * bbox[3]
                kps_xy = d["keypoints"]
