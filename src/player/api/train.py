...
@hydra.main(config_path=None, config_name="rtdetr_generic", version_base=None)
def main(cfg: DictConfig) -> None:
    """
    プレイヤー検出モデルのトレーニングを実行する

    Args:
        cfg: Hydra設定
    """
    log.info("プレイヤー検出モデルのトレーニングを開始します...")
    ...
    try:
        ...
        # チェックポイントからの再開を確認
        checkpoint_dir = cfg.get("checkpoint_dir", "checkpoints/player")
        ...
        # Hydraがネストされた設定（model, optimizer_params, scheduler_paramsなど）を自動的にインスタンス化し、
        # LitGenericPlayerModelに注入します。
