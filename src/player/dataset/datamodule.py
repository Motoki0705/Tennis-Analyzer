...
class CocoDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return self.prepare_dataloader(dataset=self.train_dataset, shuffle=True)

    def val_dataloader(self):
        return self.prepare_dataloader(dataset=self.val_dataset, shuffle=False)

    def test_dataloader(self):
        return self.prepare_dataloader(dataset=self.test_dataset, shuffle=False)
...
if __name__ == "__main__":
    ...
    for encoding in train_dataloader:
        ...
        boxes = encoding["labels"][0][
            "boxes"
        ].numpy()  # → array[N, 4], normalized [cx, cy, w, h]
