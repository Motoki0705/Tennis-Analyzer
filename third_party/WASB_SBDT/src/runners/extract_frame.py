...
class ExtractFrameRunner(BaseRunner):
    def __init__(self,
        ...
        if not self._dataset_name in ['badminton', 'soccer']:
            raise KeyError('{} does not require frame extraction : {}'.format(self._dataset_name))
    ...
    def run(self):
        if self._dataset_name=='badminton':
            ...
            raise KeyError('for this dataset extrac_frame is not defined : {}'.format(self._dataset_name))
