    ...
    for csv_path in csv_files_found: # ソート済みのリストを使う
        ...
        try:
            ...
            if not game_match or not clip_match:
                 print(f"  Warning: Could not extract game/clip ID from path structure near: {csv_path}. Expected 'gameX/ClipY' format in directory names. Skipping.")
                 continue
            game_id = int(game_match.group(1))
            clip_id = int(clip_match.group(1))
        except Exception as e:
            print(f"  Warning: Error extracting game/clip ID from path {csv_path}: {e}. Skipping.")
            continue
        ...
        for i, row in enumerate(rows):
            try:
                # --- 必須列の存在チェックと値の取得 ---
                if not all(k in row for k in ['file name', 'visibility', 'x-coordinate', 'y-coordinate', 'status']):
                    print(f"  Warning: Row {i+1} in {csv_path} is missing required columns. Skipping row.")
                    continue
                ...
                if not x_str or not y_str or not status_str:
                     print(f"  Warning: Row {i+1} in {csv_path} (file: {file_name}) has empty coordinates or status for a visible object. Skipping row.")
                     continue
                ...
                if original_path_str not in image_path_to_id:
                    try:
                        ...
                        image_path_to_id[original_path_str] = {"id": current_image_id, "width": width, "height": height}
                        image_id_counter += 1
                        # print(f"  Added image: {original_path_str} (ID: {current_image_id})") # 詳細ログ

                    except FileNotFoundError:
                        print(f"  Warning: Image file not found at {image_full_path} (referenced in {csv_path}, row {i+1}). Skipping annotation.")
                        continue # 画像がない場合はアノテーションも追加できない
                    except Exception as e:
                        print(f"  Warning: Error opening or reading image {image_full_path}: {e}. Skipping annotation.")
                        continue
                ...
                if clipped_w <= 0 or clipped_h <= 0:
                    print(f"  Warning: Calculated bounding box has zero or negative size for {original_path_str} at ({x},{y}). Skipping annotation.")
                    continue
                ...
                print(f"  Warning: Invalid numeric value in row {i+1} of {csv_path} (file: {file_name}): {e}. Skipping row.")
            except Exception as e:
                print(f"  Error processing row {i+1} in {csv_path} (file: {file_name}): {e}. Skipping row.")
