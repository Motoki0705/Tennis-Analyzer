...
class PersonCocoDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Optional[Tuple[Image.Image, int, Dict[str, Any]]]:
...
def collate_fn(batch):
    ...
    if not batch:
        return None
    images, image_ids, img_infos = zip(*batch)
    return {
        "images": list(images),
...
class PersonDetectorConditionalDETR:
    ...
    def _prepare_categories(self):
        ...
        print(f"Prepared output category id={INITIAL_CATEGORY_ID}, name='{INITIAL_CATEGORY_NAME}'")

    def _create_coco_annotation(self,
                                detection: Dict[str, Any],
                                image_id: int,
                                annotation_id: int) -> Dict[str, Any]:
    ...
    def process_images(self):
        ...
        for batch_idx, batch in enumerate(loader, start=1):
                ...
                for score, label, box in zip(res["scores"], res["labels"], res["boxes"]):
                    if label.item() == self.person_class_id:
                        ...
                        ann = self._create_coco_annotation(
                            {"bbox": [float(x) for x in bbox], "score": float(score.item())},
                            image_id=img_id,
            ...
            print(f"  Batch {batch_idx}: 処理完了 {len(pil_images)} 枚, 累計検出数 {total_detections}")
...
if __name__ == "__main__":
    ...
    OUTPUT_COCO  = "data/annotation_jsons/raw_non_player_annotations_conditional_detr_batched.json"
