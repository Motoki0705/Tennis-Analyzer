...
class CourtDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        scaled_keypoints: torch.Tensor = self.scaling_to_heatmap(arged_keypoints, self.input_size, self.heatmap_size)

        # Generate heatmaps for each keypoint
        if self.is_each_keypoint:
            heatmaps = torch.zeros((len(scaled_keypoints), *self.heatmap_size), dtype=torch.float32)
            for i, (x, y, v) in enumerate(scaled_keypoints):
                if v > 0:
                    self.draw_each_gaussian(heatmaps[i], (x.item(), y.item()), sigma=self.sigma)
            ...
            for i, (x, y, v) in enumerate(scaled_keypoints):
                if v > 0:
                    self.draw_each_gaussian(heatmap[0], (x.item(), y.item()), sigma=self.sigma)
